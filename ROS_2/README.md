...
| `drone_control` | MAVROS bridge + `offboard_keyboard` (teleop `/cmd_vel` → PX4 OFFBOARD velocity, auto arm/takeoff) |
| `drone_slam` | RTAB-Map `rgbd_odometry` + `rtabmap` visual SLAM launch/params |
...
cd ~/Surge/PX4-Autopilot && HEADLESS=1 make px4_sitl gazebo-classic_s500_depth__slam_world
